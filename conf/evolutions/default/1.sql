...
alter table alarm add constraint fk_alarm_assessment_3 foreign key (assessment_id) references assessment (id) on delete restrict on update restrict;
create index ix_alarm_assessment_3 on alarm (assessment_id);
alter table alarm add constraint fk_alarm_fieldAssessment_4 foreign key (field_assessment_id) references assessment (id) on delete restrict on update restrict;
create index ix_alarm_fieldAssessment_4 on alarm (field_assessment_id);
...
alter table assessment add constraint fk_assessment_nmi_6 foreign key (nmi_id) references nmi (id) on delete restrict on update restrict;
create index ix_assessment_nmi_6 on assessment (nmi_id);
alter table component_reading add constraint fk_component_reading_component_7 foreign key (component_id) references AALComponent (id) on delete restrict on update restrict;
create index ix_component_reading_component_7 on component_reading (component_id);
alter table field_operator_location add constraint fk_field_operator_location_fie_8 foreign key (field_operator_id) references alarm_attendant (id) on delete restrict on update restrict;
create index ix_field_operator_location_fie_8 on field_operator_location (field_operator_id);
